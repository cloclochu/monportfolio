...
import { accordionData, accordionDataEL } from "@/lib/data";
import Link from "next/link";
import { ArrowLeft } from "lucide-react";

export default function ElodieLaurentPage() {
  const projectDetails = [
    { label: "Client:", value: "Elodie Laurent" },
  ...
  return (
    <div className="flex flex-col px-6 md:px-12 lg:px-24 gap-20 bg-white min-h-screen">
      ...
      <Card className="relative h-[400px] w-full rounded-[32px] overflow-hidden border-none shadow-md bg-transparent">
        {/* 背景图片 */}
        <div
          className="absolute inset-0 bg-cover bg-[center_66%]"
          style={{
            backgroundImage: `url('/hypno.JPG')`, 
          }}
        />

        {/* 半透明遮罩 */}
        <div className="absolute inset-0 bg-[#a37b73] opacity-70" />
...
Élodie Laurent, Hypnothérapeute          </h1>
          <p className="max-w-2xl text-lg md:text-xl font-['Times_New_Roman'] tracking-wide leading-relaxed">
            Réinvention d’une présence digitale douce, mobile-first, pour une praticienne en hypnose bienveillante et moderne
          </p>
        </CardContent>
      </Card>
        ...
        <Card className="w-full md:w-[596px] bg-transparent border-none shadow-none">
          <CardContent className="px-[19px] py-[61px]">
            <p className="font-normal text-black text-xl leading-relaxed tracking-wide">
              Une refonte de l’identité numérique d’Élodie Laurent, pensée exclusivement pour le mobile, afin d’optimiser l’expérience des utilisateurs venant d’Instagram. L’objectif : créer une interface élégante, rassurante et intuitive, en cohérence avec sa pratique thérapeutique.
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Accordion Section */}
      <div className="w-full max-w-4xl mx-auto bg-[#5e4c5c80] rounded-[48px] px-12 py-8 font-['Libre_Baskerville'] border border-none ">
        <Accordion type="single" collapsible className="w-full">
          {accordionDataEL.map(item => (
        ...
        <div className="mt-6 flex justify-center">
          <a
            ...
            className="px-4 py-2 bg-[#e0e0e0] text-gray text-xs rounded-md"
          >
            Voir sur Figma
          </a>
        </div>
      ...
      >
        <Button
          className="p-2 rounded-full bg-white hover:bg-[#5e4c5c80] transition-colors"
          aria-label="Voir le projet précédent"
        >
          <ArrowLeft className="w-5 h-5 text-[#5e4c5c] group-hover:text-[#a37b7380] transition-colors" />
        </Button>
        <div>
